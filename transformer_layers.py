...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, k: Tensor, v: Tensor, q: Tensor, mask: Tensor = None):
        ...
        # we add a dimension for the heads to it beslow: [B, 1, 1, M]
...
class Custom_Attention(nn.Module):
    def __init__(self, num_heads: int, size: int, dropout: float = 0.1, sent_length: int = 128):
        ...
        self.k_layer = nn.Linear(size//4, num_heads * head_size//4)
        self.v_layer = nn.Linear(size, num_heads * head_size)
        self.q_layer = nn.Linear(size//4, num_heads * head_size//4)
    
        self.confident_layer = nn.Sequential(nn.Linear(sent_length, sent_length),nn.ReLU())
        
        self.output_layer = nn.Linear(size, size)
        self.softmax = nn.Softmax(dim=-1)
        self.dropout = nn.Dropout(dropout)

    def forward(self, k: Tensor, v: Tensor, q: Tensor, mask: Tensor = None, xy_pdf_score: Tensor = None):
        batch_size = k.size(0)
        num_heads = self.num_heads
        ...
        if mask is not None:
            scores = scores.masked_fill(~mask.unsqueeze(1), float('-inf'))
#         print(scores.shape) #xy_pdf_score
        # apply attention dropout and compute context vectors.
        if xy_pdf_score is None:
            attention = self.softmax(scores)
        else:
            xy_pdf_score = self.confident_layer(xy_pdf_score)
            xy_pdf_score = xy_pdf_score.view(batch_size,1,1,-1)
            scores_exp = scores.exp()
            new_scores = scores_exp * xy_pdf_score
            new_scores_sum = new_scores.sum(-1)
            attention = new_scores/new_scores_sum.unsqueeze(-1)
...
class LabelWiseAttention(nn.Module):
    ...
    https://github.com/OpenNMT/OpenNMT-py
    """
    def __init__(self, num_heads: int, size: int, dropout: float = 0.1, max_label = 4):
        """
        Create a multi-headed attention layer.
        :param num_heads: the number of heads
        :param size: model size (must be divisible by num_heads)
    ...
    def forward(self, k: Tensor, v: Tensor, q: Tensor, label: Tensor, mask: Tensor = None):
        """
        Computes multi-headed attention.
        :param k: keys   [B, M, D] with M being the sentence length.
        :param v: values [B, M, D]
        ...
        final_score = torch.zeros(scores.size(), device = scores.device)
        for i in range(self.max_label):
            label_mask = (label==i).unsqueeze(1)
            label_score = scores.masked_fill(~label_mask.unsqueeze(1), float(-100))
            label_score = self.softmax(label_score)
            label_score = label_score*(label_mask.unsqueeze(3).repeat(1,num_heads,1,p_num).float())
            final_score = final_score + label_score
        attention = self.dropout(final_score)
...
class PositionwiseFeedForward(nn.Module):
    ...
    def __init__(self, input_size, ff_size, dropout=0.1):
        ...
        self.layer_norm = nn.LayerNorm(input_size, eps=1e-6, elementwise_affine=False)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.layer_norm = nn.LayerNorm(size, eps=1e-6, elementwise_affine=False)
    ...
    # pylint: disable=arguments-differ
    def forward(self, x: Tensor, mask: Tensor, y = None, y_mask=None) -> Tensor:
        """
        ...
        if y != None:
            ...
            h = self.src_src_att(y_norm, y_norm, x_norm, y_mask)
        else:
...
class TransformerRefineLayer(nn.Module):
    ...
    def __init__(self,
        ...
        :param dropout: dropout to apply to input
        """
        super(TransformerRefineLayer, self).__init__()
        self.layer_norm = nn.LayerNorm(size, eps=1e-6)
        self.box_norm = nn.LayerNorm(size//4, eps=1e-6)
        self.src_src_att = Custom_Attention(num_heads, size,
                                                dropout=dropout, sent_length=sent_length)
...
class LabelWiseTransformerEncoderLayer(nn.Module):
    """
    One Transformer encoder layer has a Multi-head attention layer plus
    a position-wise feed-forward layer.
    """
    def __init__(self,
        ...
        :param dropout:
        """
        super(LabelWiseTransformerEncoderLayer, self).__init__()

        self.layer_norm = nn.LayerNorm(size, eps=1e-6, elementwise_affine=False)
        self.src_src_att = LabelWiseAttention(num_heads, size,
                                                dropout=dropout, max_label = max_label)
        self.feed_forward = PositionwiseFeedForward(size, ff_size=ff_size, dropout=dropout)
        self.dropout = nn.Dropout(dropout)
        self.size = size

    # pylint: disable=arguments-differ
    def forward(self, x: Tensor, label: Tensor, mask: Tensor) -> Tensor:
        """
        Forward pass for a single transformer encoder layer.
        First applies layer norm, then self attention,
        then dropout with residual connection (adding the input to the result),
        ...
        """
        x_norm = self.layer_norm(x)
        h = self.src_src_att(x_norm, x_norm, x_norm, label, mask)
        h = self.dropout(h) + x
        o = self.feed_forward(h)
        return o
